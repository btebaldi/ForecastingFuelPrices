...
dim(const mMatrix, const label = "matrix")
{ println(label,"=",rows(mMatrix), " x ", columns(mMatrix)); }
...
main()
{
	...
	println("Script responsavel pelo calculo das matrizes G \n");
	...
	mG_alpha = loadmat(sprint(txMatPathG_Matrix, "G_alpha_v2.mat"));
	...
	mL = mAlphaBeta_w ~ zeros(rows(mAlphaBeta_w), (columns(mG_alpha)-columns(mAlphaBeta_w)));
	mL = mL | mG_alpha;
