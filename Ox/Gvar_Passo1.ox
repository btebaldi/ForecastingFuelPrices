...
main() {
	...
	println("Script responsavel pela estimacao dos modelos individuais \n");
	println("GVAR Passo 1 Inicializado");
    /***************************************************
     *
    ...
    for (iCont = 91; iCont <= iQtdRegioes; ++iCont) {
		...
		println("%c", {"ETA", "DIS", "GAS", "ETA*", "DIS*", "GAS*", "brent", "FX"}, "%r", {"T_1", "T_2", "T_3", "T_4"}, mData[0:5][]);
		println("%r", {"ETA", "DIS", "GAS", "ETA*", "DIS*", "GAS*", "brent", "FX"}, "%c", {"T_1", "T_2", "T_3", "T_4"}, beta);
...
//		model.SaveIn7(sprint("BASEDEBUG_", iCont, "_Fulldatabase"));
		...
		********* */
		...
		if (iQtdLags > 0) {
			...
			model.Select("U", {sprint(sVarSufix, "R_", iCont, "_", aVarDependenteNames[iContador]), 1, iQtdLags});
	        }
        }
		...
		for(iContador = 0; iContador < columns(aVarDependenteNames); ++iContador) {
            // Adiciona a variavel em primeira Diferenca
			println("\tAdicionando variavel star da regiao ", iCont, " (", aVarDependenteNames[iContador],")");
			model.Select("U", {sprint(sVarSufix, sprint("star_", aVarDependenteNames[iContador])), 0, iQtdLags});
	    }
	
		// Adiciona a variavel "betaZ" com um lag apenas (representação da matriz de longo prazo)
		model.Select("U", asBetaZ);															 
        ...
        //decl nContVarMacro;
        for (i = 0; i < rows(aMacroVarNames); ++i) {
            model.Select("U", {sprint(sVarSufix, aMacroVarNames[i]), 0, iQtdLags});
